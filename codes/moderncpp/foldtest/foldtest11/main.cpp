...
bool plot(const A &a, const B &b, const std::string &format, const Args&... args)
{
...
template <typename A, typename B, typename... Args>
bool plot(const A &a, const B &b, const std::map<std::string, std::string> &keywords, const Args&... args)
{
    std::cout << "plot 4444 number of args = " << sizeof ... (args) << std::endl;
    plot(a, b, keywords ) && plot( args... );
    return true;
}
...
int main( int argc, char **argv )
{
    ...
    plot( i, j, "cfd", i, j, keywords );
    plot( i, j, "cfd", i, j, "OneFLOW", i, j, "cfd" );
    plot( i, j, keywords, i, j, keywords, i, j, keywords );
    return 0;
}

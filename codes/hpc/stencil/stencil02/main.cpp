        ...
        for (int xBase = 0; xBase < nx; xBase += blockSize) {
            for (int y = 0; y < ny; y++) {
                for (int xTmp = xBase; xTmp < xBase + blockSize; xTmp += 16) {
                    _mm_prefetch((const char*)(&a(xTmp, y + nblur)), _MM_HINT_T0);
        ...
        for (int xBase = 0; xBase < nx; xBase += blockSize) {
            for (int y = 0; y < ny; y++) {
                for (int x = xBase; x < xBase + blockSize; x += 16) {
                    _mm_prefetch((const char*)(&a(x, y + nblur)), _MM_HINT_T0);
                    float res[16];
...
#pragma omp parallel for collapse(2)
        for (int x = 0; x < nx; x += 32) {
            for (int y = 0; y < ny; y++) {
                _mm_prefetch((const char*)(&a(x, y + nblur)), _MM_HINT_T0);
                _mm_prefetch((const char*)(&a(x + 16, y + nblur)), _MM_HINT_T0);
                __m256 res[4];
#pragma GCC unroll 4
                for (int offset = 0; offset < 4; offset++) {
...
#pragma omp parallel for collapse(2)
        for (int x = 0; x < nx; x += 32) {
            for (int y = 0; y < ny; y++) {
                _mm_prefetch((const char*)(&a(x, y + nblur + 40)), _MM_HINT_T0);
                _mm_prefetch((const char*)(&a(x + 16, y + nblur + 40)), _MM_HINT_T0);
                __m256 res[4];
#pragma GCC unroll 4
                for (int offset = 0; offset < 4; offset++) {
                    res[offset] = _mm256_setzero_ps();

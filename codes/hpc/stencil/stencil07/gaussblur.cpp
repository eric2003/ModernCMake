...
#pragma omp parallel for collapse(2)
        for (int y = 0; y < ny; y++) {
            for (int xBase = 0; xBase < nx; xBase += 16) {
                _mm_prefetch(reinterpret_cast<const char*>(&a(xBase + 16, y, comp)), _MM_HINT_T0);
...
#pragma omp parallel for collapse(2)
        for (int x = 0; x < nx; x += 32) {
            for (int y = 0; y < ny; y++) {
                _mm_prefetch(reinterpret_cast<const char*>(&a(x, y + nblur + 40, comp)), _MM_HINT_T0);
                _mm_prefetch(reinterpret_cast<const char*>(&a(x + 16, y + nblur + 40, comp)), _MM_HINT_T0);
                __m256 res[4];
#ifdef _MSC_VER
#pragma unroll 4
#else

...
int main( int argc, char **argv )
{
    ...
    }
    {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, n),
        [&] ( tbb::blocked_range<size_t> r ) {
            std::vector<float> local_array;
            local_array.reserve( r.size() );
            for ( size_t i = r.begin(); i < r.end(); ++ i )
            {
                float val = std::sin(i);
                if ( val > 0 )
                {
                    local_array.push_back(val);
                }
            }
            std::lock_guard lck(mtx);
            std::copy( local_array.begin(), local_array.end(), std::back_inserter(g) );
        });
    ...
    }
    {
        ...
        [&] ( tbb::blocked_range<size_t> r ) {
            ...
            std::lock_guard lck(mtx);
            std::copy( local_array.begin(), local_array.end(), std::back_inserter(my_array) );
        });
        std::cout << "h.size() = " << my_array.size() << std::endl;
        tbb::tick_count t1 = tbb::tick_count::now();
        std::cout << "time elapsed: " << ( t1 - t0 ).seconds() * 1000 << " ms" << std::endl;
    }
